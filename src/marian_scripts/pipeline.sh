...
train() {
	...
	do
		...
		if [[ ! -f $model_file ]]; then
			...
			python $POSTPROCESS_PY -i ${output_file} -o ${output_file}.noref -v $var_rewrite -s $sig_file --no_referee -rcl $remove_clauses -se $sep --no_sep > $log_noref
